...
const CoinItem = ({token, sender, setAction, selectedToken, setSelectedToken, sanityTokens, thirdWebTokens, walletAddress}) => {
    ...
    useEffect(() => {
        const getBalance = async() => {
            ...
            thirdWebTokens.map((thirdWebToken, index) => {
                if(thirdWebToken.address === token.contractAddress){
                    activeThirdWeb = thirdWebToken
                }
            })

            const balance = await activeThirdWeb.balanceOf(sender)
